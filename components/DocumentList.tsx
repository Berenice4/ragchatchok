                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-labelledby="upload-doc-title">
                    <div className="bg-gem-slate p-6 rounded-lg shadow-xl w-full max-w-lg">
                        ...
                        <div className="mb-4">
                            ...
                            />
                            {selectedFile && <p className="text-sm mt-2 text-gem-offwhite/70">Selezionato: {selectedFile.name}</p>}
                        </div>

                        <div className="mb-4">
                            <h4 className="text-sm font-medium text-gem-offwhite/80 mb-2">Metadati Personalizzati (opzionale)</h4>
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                {metadata.map((item, index) => (
                                    <div key={index} className="flex items-center space-x-2">
                                        <input type="text" placeholder="Chiave" value={item.key} onChange={(e) => handleMetadataChange(index, 'key', e.target.value)} className="w-1/2 bg-gem-mist border border-gem-mist/50 rounded-md py-1 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-gem-blue" />
                                        <input type="text" placeholder="Valore" value={item.value} onChange={(e) => handleMetadataChange(index, 'value', e.target.value)} className="w-1/2 bg-gem-mist border border-gem-mist/50 rounded-md py-1 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-gem-blue" />
                            ...
                            <button onClick={addMetadataRow} className="mt-2 flex items-center text-sm text-gem-blue hover:text-blue-400" title="Aggiungi un altro campo metadati">
                                <PlusIcon /> <span className="ml-1">Aggiungi Metadati</span>
                            </button>
                        </div>
                        
                        <div className="flex justify-end space-x-2 mt-6">
                            <button type="button" onClick={handleModalClose} className="px-4 py-2 rounded-md bg-gem-mist hover:bg-gem-mist/70 transition-colors" title="Annulla caricamento">
                                Annulla
                            </button>
                            <button type="button" onClick={handleConfirmUpload} disabled={!selectedFile} className="px-4 py-2 rounded-md bg-gem-blue hover:bg-blue-500 text-white transition-colors disabled:bg-gem-mist/50 disabled:cursor-not-allowed" title="Carica file selezionato">
                                Carica
                            </button>
                        </div>
                    </div>
                        ...
                        <li key={doc.name} className="p-3 bg-gem-mist rounded-md group">
                             ...
                             {doc.customMetadata && doc.customMetadata.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-gem-mist/50 text-xs">
                                    <h4 className="font-semibold text-gem-offwhite/70 mb-1">Metadati:</h4>
